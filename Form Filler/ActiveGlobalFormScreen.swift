...
struct ActiveGlobalFormScreen: View {
    ...
    var body: some View {
        Form {
            ...
            Section(header: Text("Vitals")) {
                HStack {
                    TextField("BP", text: $agVM.data.bp)
                        .onChange(of: agVM.data.bp) { agVM.save() }
                    TextField("SpO₂", text: $agVM.data.spo2)
                        .onChange(of: agVM.data.spo2) { agVM.save() }
                }
                HStack {
                    TextField("PR", text: $agVM.data.pr)
                        .onChange(of: agVM.data.pr) { agVM.save() }
                    TextField("Hypocount", text: $agVM.data.hypocount)
                        .onChange(of: agVM.data.hypocount) { agVM.save() }
                }
            }
            ...
            .onChange(of: agVM.data.pastMedicalHistory) { agVM.save() }
            .onChange(of: agVM.data.presentingComplaint) { agVM.save() }
            .onChange(of: agVM.data.physicalExam) { agVM.save() }
            .onChange(of: agVM.data.diagnosisIssues) { agVM.save() }
            .onChange(of: agVM.data.managementPlan) { agVM.save() }

            Section(header: Text("Clinician")) {
                TextField("Name", text: $agVM.data.clinicianName)
                    .onChange(of: agVM.data.clinicianName) { agVM.save() }
                TextField("MCR", text: $agVM.data.clinicianMCR)
                    .textInputAutocapitalization(.characters)
                    .onChange(of: agVM.data.clinicianMCR) { agVM.save() }
